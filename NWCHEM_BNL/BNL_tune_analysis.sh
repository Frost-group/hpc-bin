...
#Believe we just want the Alpha orbitals?
    ...
    echo -n "${i} ${CAM} "
echo "NEUTRAL"
    grep "Vector\s\+${ELECTRONS}" "${i}" | sed -n 1p | sed s/=/\ /g | sed s/D/E/g | awk '{printf(" %f ",$6*27.211)}'
let LUMO=${ELECTRONS}+1
    grep "Vector\s\+${LUMO}" "${i}" | sed -n 1p | sed s/=/\ /g | sed s/D/E/g | awk '{printf(" %f ",$6*27.211)}'
echo "CATION"
#    grep "Vector\s\+${ELECTRONS}" "${i}" | sed -n 2p | sed s/=/\ /g | awk '{printf (" %f ",$6*0.27211)}'
grep "Total DFT energy" "${i}" | sed -n 2p | awk '{printf (" %f ",27.211*-($5-('$ENERGY')))}'
echo "ANION"
#    grep "Vector\s\+${ELECTRONS+1}" "${i}" | sed -n 4p | sed s/=/\ /g | awk '{printf (" %f ",$6*0.27211)}'
